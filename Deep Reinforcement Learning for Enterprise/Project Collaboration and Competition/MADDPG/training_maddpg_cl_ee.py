...
def maddpg(env, dic = {'n_episodes': 1000,
       'max_t': 300,
       'actor_fc1_units': 400,
       'actor_fc2_units': 300,
       ...
       'file': 'chpt',
       }, SuccessStop=True, NoiseNet=True, PER=True):
    ...
    team = Team(num_agents=num_agents, state_size=state_size, action_size=action_size, hyper_dic=dic, PER=PER)
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for t in range(max_t):
            actions = team.act(states, add_noise=True, episode=i_episode, NoiseNet=NoiseNet)
            env_info = env.step(actions)[brain_name]        # send the action to the environment
            next_states = env_info.vector_observations   # get the next state
            rewards = env_info.rewards                   # get the reward
            dones = env_info.local_done
            team.step(states, actions, rewards, next_states, dones)
            states = next_states
            scores += rewards
            if np.any(dones):
                break 
        ...
        if i_episode % 100 == 0:
            ...
            if np.mean(scores_window)>=0.5:
                for i, agent in enumerate(team.maddpg_agents):
                    torch.save(agent.actor_local.state_dict(), file+'_actor_'+str(i)+'.pth')
                    torch.save(agent.critic_local.state_dict(), file+'_critic_'+str(i)+'.pth')
